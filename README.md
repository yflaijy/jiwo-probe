...
- **系统历史曲线**——通用二级详情页（像素、扁平、动漫、玻璃、Lumina、Glassmorphism、Emerald）的趋势栏提供「CPU」「内存」「TCP/UDP」：连接数以 TCP／UDP 双线显示，支持分别隐藏曲线、1h/6h/24h 档位与缩放适应。直接读取主控 series `metric=system`，连接数显示时四舍五入为整数，原始桶平均精度不变，缺失不补零；未配置 Ping 的服务器也可查看。Lite、LuminaPlus 保留「系统」页中的连接数双线图。
